Module Module1
    ...
    Sub Main()
        ...
        '        MsgBox(Console.Read())
        Console.WriteLine("引擎启动完毕 *test 0.1")

yinyuanlujing:
        Console.WriteLine("输入音源文件夹路径 *test 0.1")
        mSingerdir = Console.ReadLine
...
U:
        ...
        UST(mUstname, mJichu, mSingerdir, mNotenum)
        'MADD(mFen)
        MADD(mFen)
    ...
    End Sub
    Sub UST(ByVal UstName As String, ByVal Jichu As Short, ByVal SingerDir As String, ByVal Notenums As Integer)
        'UST 
        'UstName 文件路径
...
P:
        ...
        If Lyric <> "R" Then
            Try
                ...
                A = LL.LengthChange(1, SingerDir & "\SoundSource\" & wav, SLength, SingerDir & "\Cache\" & No & ".wav", mFen, start, Send, consonant, SingerDir & "\Cache\")
                A = LP.Pitch(1, SingerDir & "\Cache\", No & ".wav", SingerDir & "\Cache2\" & No + 1 & ".wav", Tall, 0.7)
        ...
        Else
            ...
            If Dir(SingerDir & "\Cache\" & No & ".wav") = "" Then
                ...
                A = LL.LengthChange(1, SingerDir & "\Cache\" & No & ".wav", SLength, SingerDir & "\Cache2\" & No + 1 & ".wav", mFen, start, Send, consonant, SingerDir & "\Cache\")
                ' Kill(SingerDir & "\Cache\" & No & ".wav")
                'If No = 3 Then
                'A = LA.Add(1, SingerDir & "\Cache2\" & No - 1 & ".wav", SingerDir & "\Cache2\" & No & ".wav", SingerDir & "\Cache3\a.wav")
                ' End If
            End If
            A = LL.LengthChange(1, SingerDir & "\SoundSource\" & wav, SLength, SingerDir & "\Cache2\" & No + 1 & ".wav", mFen, start, Send, consonant, SingerDir & "\Cache\")
        End If
        '  mFen = 3
        ...
        End If
    End Sub
    Sub MADD(ByVal fen As Single)
        Console.WriteLine("合成第 1 个")
        Dim fenno As Long = fen
        A = LA.Add2(1, mSingerdir & "\Cache2\3.wav", mSingerdir & "\Cache2\4.wav", mSingerdir & "\Cache3\1.wav", fenno)
        fenno = fenno + fen
...
'LCM可调参数列表
'
'Fade Fraction Value（FFV 淡入淡出分数值，与UTAU中重叠相似）
'
'
'
'
